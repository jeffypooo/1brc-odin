...
main :: proc() {
    ...
    when USE_MMAP {
        ...
        when ODIN_OS == .Darwin || ODIN_OS == .Linux {
            bytes_ptr: rawptr = posix.mmap(nil, uint(file_size), { .READ }, { .SHARED }, posix.FD(fd), 0)
...
write_to_str :: proc(values: []Key_Value) -> string {
    ...
    for entry, index in values {
        ...
        strings.write_string(&sb, fmt.tprintf("%.1f/%.1f/%.1f", entry.value.min_temp, avg_temp, entry.value.max_temp))
